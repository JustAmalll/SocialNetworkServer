...
import dev.amal.data.requests.LikeUpdateRequest
...
fun Route.unlikeParent(
    ...
    authenticate {
        delete("/api/unlike") {
            ...
            val parentType =
                call.parameters[QueryParams.PARAM_PARENT_TYPE]?.toIntOrNull() ?: kotlin.run {
                    call.respond(HttpStatusCode.BadRequest)
                    return@delete
                }
...
fun Route.getLikesForParent(likeService: LikeService) {
    authenticate {
        get("/api/like/parent") {
            ...
            call.respond(HttpStatusCode.OK, usersWhoLikedParent)
        }
    }
}